...
var debug = require('debug')('pancake:sorted-articles');
...
function sortedArticles (req, res, next) {
  debug('populating `req.sorted_articles` (%s)', req.sha);
  ...
  function sort () {
    debug('sorting array of %d "article objects" by their "date"', articles.length);
