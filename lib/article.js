...
var debug = require('debug')('pancake:article');
...
function article (req, res, next) {
  ...
  var name = req.params.article[0];
  //if ('/' !== name[name.length - 1]) {
  //  debug('skipping non-article request, no trailing "/" (%s) %j', req.sha, name);
  //  return next();
  //}
  ...
  debug('got article request %j', name);
